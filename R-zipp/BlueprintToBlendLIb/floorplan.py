...
def new_floorplan(img_path, config=None):
    """
    Creates and returns a new floorplan class from config file
    """
    return floorplan(img_path, conf=config)
...
class floorplan:
    ...
    def __init__(self, img_path, conf=None):
        if conf is None:
            # use default
            conf = const.IMAGE_DEFAULT_CONFIG_FILE_NAME
        self.conf = conf
        self.img_path = img_path.replace('\\', '/')
    ...
    def create_variables_from_config(self, conf):
        ...
        settings_dict['IMAGE']['image_path'] = f'"{self.img_path}"'
